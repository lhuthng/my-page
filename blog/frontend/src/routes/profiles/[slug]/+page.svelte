	...
	<PBody>
		<div class="sticky top-0 flex justify-center items-center w-full h-screen pointer-events-auto">
			...
			<div class="w-fit h-fit space-y-4 bg-white rounded-3xl p-4 text-xl z-11" role="none">
				...
				>
					{#if editor.newAvatar}
						<img class="full object-cover" src={editor.newAvatar} alt="temporal-avatar" />
					{:else}
				...
				<div class="duo-btn" data-duo-color="green">
					<button
						...
						onclick={async () => {
							...
							formData.append('file', editor.avatarFile, editor.avatarFile.name);
				...
				{#if editor.isUploaded}
					<span class="inline-block w-60 text-accent-green">*New avatar uploaded succesfully!</span>
				{/if}
			</div>
		</div>
	</PBody>
...
<section class="flex flex-col gap-4 *:bg-white *:rounded-xl *:p-4 pb-8" key={username}>
						...
						<div class="flex items-start gap-3">
							...
							<div class="min-w-0 grow">
								<div class="flex flex-wrap items-baseline gap-x-2 gap-y-1">
									<a
										class="font-semibold text-dark hover:text-primary"
	...
	{#if hasPosts}
		<div class="flex flex-col gap-4">
			...
			{#if posts.status === 'fetching'}
				<div class="flex justify-center items-center p-4 text-xl">Loading...</div>
			...
			{:else if posts.status === 'fetched'}
				...
				>
					{#each posts.data as { id, title, slug, excerpt, author_name, author_slug, tag_slugs, status, url, stats, cover_media_type, reading_time_minutes }, index (slug)}
