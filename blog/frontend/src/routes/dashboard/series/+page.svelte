	...
	{:else}
		<ul class="flex flex-col gap-3">
			{#each series as s (s.id)}
				<li class="bg-white rounded-xl overflow-hidden" in:fly={{ y: -10, duration: 300 }}>
					<!-- Series header row -->
					<div class="flex items-center gap-4 p-4">
						...
						<div class="flex flex-col min-w-0 grow">
							<div class="flex items-center gap-2 flex-wrap">
								...
								{#if s.owner_username}
									<span class="text-xs bg-primary/20 px-2 py-0.5 rounded-full text-dark/60">
										by {s.owner_username}
									</span>
								{/if}
							</div>
					...
					{#if expanded[s.id]}
						<div class="border-t border-background px-4 pb-4" in:fly={{ y: -8, duration: 200 }}>
							...
							{:else if seriesPosts[s.id]?.status === 'error'}
								<p class="py-4 text-center text-accent-red">Failed to load posts.</p>
									...
									{#each seriesPosts[s.id].items as post (post.post_id)}
										<li class="flex items-center gap-3 py-2" out:fade={{ duration: 150 }}>
											...
											{#if post.cover_url}
												...
												/>
											{:else}
												<div class="w-10 h-10 rounded bg-primary/20 shrink-0"></div>
											{/if}
											<div class="flex flex-col min-w-0 grow">
												<a
