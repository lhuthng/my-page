...
<div class="flex flex-col gap-4 pb-8">
  ...
  <div class="bg-white rounded-xl p-6 shadow-sm">
    ...
    <p class="text-base text-dark/60 mt-1">
      Select which posts are featured in the "Discover" tab on the homepage. The discover section on the homepage displays the 5 most recent featured posts.
    </p>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
    ...
    <div class="lg:col-span-7 bg-white rounded-xl p-6 shadow-sm flex flex-col gap-4 h-fit">
      ...
      {#if featuredPosts.length === 0}
        <div class="flex flex-col items-center justify-center py-16 text-dark/40 border-2 border-dashed border-background/60 rounded-xl" in:fade>
        ...
        <ul class="flex flex-col gap-3">
          {#each featuredPosts as post (post.id)}
            ...
            >
              ...
              {:else}
                <div class="w-16 h-16 rounded-lg bg-background/40 shrink-0 flex items-center justify-center text-dark/30 text-xs font-semibold">
                  No Cover
                </div>
              {/if}
              
              <div class="flex-1 min-w-0">
                <a href="/posts/{post.slug}" target="_blank" class="font-bold text-lg text-dark hover:text-primary hover:underline truncate block">
                  {post.title}
                </a>
                <p class="text-sm text-dark/50 mt-0.5">by {post.author_name} · @{post.author_slug}</p>
              ...
              <div class="duo-btn duo-red shrink-0">
                <button onclick={() => toggleHighlight(post, true)} class="px-3 py-1.5 text-sm font-semibold">
                  Remove
                </button>
              </div>
            </li>
    ...
    <div class="lg:col-span-5 bg-white rounded-xl p-6 shadow-sm flex flex-col gap-4">
      ...
      <!-- Search input field -->
      <div class="relative flex items-center gap-2 bg-background/30 rounded-xl px-3 py-2.5 border border-background/60">
        ...
        {:else}
          <ul class="flex flex-col gap-3" in:fade>
            {#each searchResults as post (post.id)}
              ...
              <li class="flex items-center gap-3 p-3 border border-background/40 rounded-xl hover:bg-background/5 transition-colors">
                {#if post.url}
                  <img src={post.url} alt="" class="w-12 h-12 rounded-lg object-cover shrink-0" />
                {:else}
                  <div class="w-12 h-12 rounded-lg bg-background/40 shrink-0 flex items-center justify-center text-dark/30 text-xs font-semibold">
                    No Cover
                  </div>
                {/if}
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center gap-2 mt-1 text-xs">
                    <span class="px-1.5 py-0.5 rounded-full {isDraft ? 'bg-accent-yellow/20 text-dark/70' : 'bg-accent-green/20 text-accent-green'} font-semibold">
                      {post.status}
                    </span>
                    {#if isDraft}
                      <span class="text-accent-red font-medium">Draft (Will not display on homepage)</span>
                    {/if}
                  </div>
                </div>
                ...
                <div class="shrink-0">
                  {#if isFeatured}
                    <div class="duo-btn duo-red">
                      <button onclick={() => toggleHighlight(post, true)} class="px-2.5 py-1.5 text-xs font-bold">
                        Remove
                      </button>
                    </div>
                  {:else}
                    <div class="duo-btn duo-green">
                      <button onclick={() => toggleHighlight(post, false)} class="px-2.5 py-1.5 text-xs font-bold">
                        Highlight
                      </button>
                    </div>
                  {/if}
