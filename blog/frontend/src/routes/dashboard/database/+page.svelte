<script>
	...
	function navigate(params = {}) {
		...
		const id = params.includeDeleted !== undefined ? params.includeDeleted : includeDeleted;
	...
	>
		...
		{#each TABLES as t}
			...
			<button
               ...
               {active ? 'bg-dark text-white' : 'text-dark hover:bg-background/60'}"
			>
				<span>{t.label}</span>
				<span class="text-xs tabular-nums {active ? 'text-white/60' : 'text-dark/40'}">
					{count}
				</span>
			</button>
		{/each}
