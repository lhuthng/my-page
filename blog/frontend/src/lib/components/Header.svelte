	...
	const search = async (type, term) => {
		...
		if (type === 'User') {
			...
			if (res.ok) {
				...
				searchData.result = { users: [] };
			}
		} else if (type === 'Tag') {
			const res = await fetch(`/api/tags?term=${encodeURI(term)}&size=5`, {
				method: 'GET'
			});
		...
		<div class="relative grow not-lg:hidden">
			...
			>
				...
				<div class="flex items-center h-full cursor-pointer">
					<div
						class="h-full px-2 border-l-2 rounded-r-lg cursor-pointer bg-white hover:bg-dark text-dark hover:text-white"
					>
						<select
							class="full text-current outline-none cursor-pointer"
							name="search-options"
							onchange={change}
						>
							<option>Post</option>
			...
			<div class="w-full flex not-md:flex-col">
				<div class="flex flex-col gap-2 grow">
					...
					>
						...
						<div class="min-w-fit flex items-center h-full cursor-pointer">
							<div
								class="h-full px-2 border-l-2 rounded-r-lg text-dark bg-white hover:text-white hover:bg-dark"
							>
								<select
									class="full text-current outline-none cursor-pointer"
									name="search-options"
				...
				<div class="w-1/2 flex not-md:flex-col justify-evenly">
					...
					<div class="flex flex-col p-2">
						...
						<ul
							class="flex flex-col gap-1 [&>li]:flex [&>li]:gap-1 [&>li]:items-center [&>li>a]:hover:fill-primary [&>li>a]:w-7"
						>
							<li>
								<FacebookButton as="a" class="fill-dark" href="https://www.facebook.com/lhuthng/" />
								<a href="https://www.facebook.com/lhuthng/">Facebook</a>
							</li>
							<li>
								<GithubButton as="a" class="fill-dark" href="https://github.com/lhuthng" />
		...
		>
			...
			{:else if searchData.status === 'fetched'}
				{#if searchData.selection === 'User'}
					{#if !searchData.result || searchData.result.users.length === 0}
						<span>
				...
				{:else if searchData.selection === 'Tag'}
					{#if !searchData.result || searchData.result.tags.length === 0}
						<span>
							No tag matches "{searchData.term}"
						</span>
					{:else}
						<ul class="space-y-2">
							{#each searchData.result.tags as { name, slug, post_count }, index (slug)}
								<li class="flex gap-4 items-center justify-between">
									<a href={'/tags/' + slug}>#{slug}</a>
									<span class="text-xs text-dark/40">
										{name}
									</span>
									<span class="text-sm text-dark/50 whitespace-nowrap">
				...
				{:else if searchData.selection === 'Post'}
					{#if !searchData.result || searchData.result.posts.length === 0}
						<span>
