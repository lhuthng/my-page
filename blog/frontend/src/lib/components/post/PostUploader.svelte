<script>
	...
	const getTextColor = (category) => (colorMap[category] ?? colorMap.default).text;
	const getBackgroundColor = (category) => (colorMap[category] ?? colorMap.default).bg;
	...
	const placeholder = pick([
		...
		['Why This Topic Is Definitely Important (Probably)', 'topic-is-important-probably'],
		['Secrets No One Asked For but Here They Are', 'secrets-nobody-asked-for'],
		['How to Be Amazing at Something You Haven’t Tried Yet', 'be-amazing-at-anything'],
		['The Ultimate Guide to Whatever This Post Is About', 'ultimate-guide-to-whatever']
	]);
	...
	let offlineMedia = $derived(
		Object.fromEntries(Object.entries(offlineMediaData).map(([key, value]) => [key, value.url]))
	);
	...
	async function post() {
		...
		const selectedCategories = categories.all.filter((c) => c.selected).map((c) => c.slug);
		...
		<div class="flex space-x-2">
			...
			<div id="categories" class="px-2">
				{#if draft.categories.status === 'successful'}
					<ol class="flex flex-wrap items-center gap-x-2 gap-y-1 w-65">
							...
							<li>
								<button
									...
									style:border-color={category.selected
										...
										: 'black'}
									style:color={category.selected ? getTextColor(category.slug) : 'black'}
									style:background-color={category.selected
										? getBackgroundColor(category.slug)
										: 'unset'}
									onclick={() =>
										(draft.categories.all[index].selected = !draft.categories.all[index].selected)}
								>
									{category.slug}
								</button>
							</li>
