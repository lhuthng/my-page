    ...
    >
      ...
      {#if (comment.direct_reply_count ?? 0) > 0}
        <div class="ml-6 lg:ml-16 mt-2 flex items-center gap-1.5 text-xs text-dark/70">
          ...
          >
            {thread.expanded ? "Hide replies" : `View ${replyCountLabel(comment.direct_reply_count)}`}
          </button>
        </div>
      {/if}

      {#if thread.expanded}
        <div class="relative ml-8 lg:ml-18 mt-2 pl-2" in:fly={{y: 10, duration: 500}} out:fade={{duration: 200}}>
