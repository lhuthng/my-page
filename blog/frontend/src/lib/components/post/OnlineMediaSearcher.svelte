...
{#snippet printResults(cache, keyword)}
	...
	<div class="p-2 grow min-h-0 max-h-full custom-scrollbar">
		{#if results !== undefined}
			{#if results.length === 0}
				<span class="block text-center">There is no medium matching "{keyword}"</span>
			{:else}
				<ul class="space-y-2">
					{#each results as { short_name: shortName, url, file_type: type }}
						<EditorMediumEntity {shortName} {url} {type} />
					{/each}
				</ul>
			{/if}
		{:else}
...
<div {...rest}>
	<div class="mx-auto w-full p-2">
		<div class="flex items-center rounded-full w-full h-8 px-2 border-2 border-primary">
			<input
				id="media-searcher"
