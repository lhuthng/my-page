<script>
	...
	async function search(keyword) {
		...
		if (requestCache[keyword] === undefined) {
			...
			try {
				const data = await api.get(`media?term=${keyword}&size=10`);
				req.status = 'success';
...
<div {...rest}>
	<div class="mx-auto p-4">
		<div class="flex items-center rounded-full w-full h-8 px-2 border border-gray-400">
			<input
	...
	<div class="grow p-2 overflow-y-scroll">
		{#if results !== undefined}
			{#if results.length === 0}
				<span class="block text-center">There is no medium matching "{deKeyword}"</span>
			{:else}
				<ul class="space-y-2">
			...
			{/if}
		{:else}
			<span class="block text-center">Enter atleast 2 characters to search media.</span>
		{/if}
	</div>
</div>
