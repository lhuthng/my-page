...
<div class="flex w-full h-full gap-2 bg-gray-100 rounded-sm p-1">
	...
	<img class="w-20 h-20 rounded-sm ring-1 object-contain" src={dictionary[name]} alt={name} />
	<div class="flex flex-col-reverse justify-between grow p-1">
		<div class="flex gap-4">
			<button
				...
				onclick={() => handlers?.startRenaming(index)}
			>
				edit
			</button>
			<button
			...
			{#if isRenaming}
				<button
					class="text-blue-500 cursor-pointer hover:brightness-140"
					onclick={() => handlers?.submitRename(index, newName)}
				>
					ok
				</button>
			{/if}
	...
	>
		...
		<svg
			...
			xmlns="http://www.w3.org/2000/svg"
			aria-hidden="true"
			class={index < length - 1 ? 'cursor-pointer hover:scale-110' : 'opacity-30'}
