	...
	async function onsubmit() {
		draft.forEach(async (alias) => {
			switch (alias.action) {
				...
				case 'modified': {
					const res = await fetch(`/api/media/d/${shortName}/aliases/${alias.origin}`, {
						method: 'PATCH',
						headers: {
							'Content-Type': 'application/json',
							Authorization: auth()
...
{#snippet remove(cls, onclick, undo)}
	<button type="button" class={cls} {onclick} title="remove">
		<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 24 24" fill="none">
			<path
				d="M7 4a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v2h4a1 1 0 1 1 0 2h-1.069l-.867 12.142A2 2 0 0 1 17.069 22H6.93a2 2 0 0 1-1.995-1.858L4.07 8H3a1 1 0 0 1 0-2h4V4zm2 2h6V4H9v2zM6.074 8l.857 12H17.07l.857-12H6.074zM10 10a1 1 0 0 1 1 1v6a1 1 0 1 1-2 0v-6a1 1 0 0 1 1-1zm4 0a1 1 0 0 1 1 1v6a1 1 0 1 1-2 0v-6a1 1 0 0 1 1-1z"
				fill="#0D0D0D"
			/>
			{#if undo}
				<path d="M4 4 L20 20" stroke="#0D0D0D" stroke-width="2" stroke-linecap="round" />
			{/if}
		</svg>
	</button>
{/snippet}
...
<form class="flex flex-col space-y-3" type="submit" {onsubmit}>
								...
								onclick={(e) => {
									...
									if (alias.origin) {
										if (alias.action !== 'deleted') {
											...
											alias.action = alias.origin === alias.name ? undefined : 'modified';
										}
									} else {
	...
	<button class="ml-auto w-fit border-2 px-1 rounded-lg cursor-pointer" type="submit">Apply</button>
</form>
