...
function mediaWithShortcutPlugin(md, options) {
	...
	md.renderer.rules.extra = (tokens, idx) => {
		...
		switch (src) {
			case void 0: return `<span class="missing-image">${value}</span>`;
			case null: return `<span class="loading-image">${value}</span>`;
			default: {
				const style = [];
				...
				switch (tag) {
					case "img": return `<img src="${src}" alt="${value}" ${styleAttr}/>`;
					case "img-inl": return `<img class="inline-block align-bottom" src="${src}" alt="${value}" ${styleAttr}/>`;
					case "img-left-float":
						const floatStyle = style.length ? `style="${style.join(";")}; float: left; margin-right: 5px; margin-bottom: 5px;"` : "style=\"float: left; margin-right: 5px; margin-bottom: 5px;\"";
						return `<img src="${src}" alt="${value}" ${floatStyle}/>`;
					default: return `<span class="invalid-tag">${tag}-${value}</span>`;
				}
			}
		}
	};
}
function youtubeBlockPlugin(md) {
...
//#endregion
export { mediaWithShortcutPlugin, youtubeBlockPlugin };
//# sourceMappingURL=index2-DUFc1Uun.js.map