...
//#endregion
//#region .svelte-kit/adapter-bun/chunks/utils2.js
	...
	#traverse_effect_tree(root, target) {
		...
		while (effect !== null) {
			...
			if (!skip && effect.fn !== null) {
				...
				else if ((flags & EFFECT) !== 0) target.effects.push(effect);
...
function schedule_effect(signal) {
	...
	while (effect.parent !== null) {
		effect = effect.parent;
		var flags = effect.f;
		if (is_flushing && effect === active_effect && (flags & BLOCK_EFFECT) !== 0) return;
...
function update_derived(derived) {
	...
	if (batch_values !== null) batch_values.set(derived, derived.v);
	else {
		...
		if (dependencies !== null) {
			...
			if ((is_disconnected || is_unowned_connected) && (active_effect === null || (active_effect.f & DESTROYED) === 0)) {
				...
				for (i = 0; i < length; i++) {
					...
					if (is_disconnected || !dependency?.reactions?.includes(derived)) (dependency.reactions ??= []).push(derived);
				}
				if (is_disconnected) derived.f ^= DISCONNECTED;
				if (is_unowned_connected && parent !== null && (parent.f & UNOWNED) === 0) derived.f ^= UNOWNED;
			}
			for (i = 0; i < length; i++) {
				dependency = dependencies[i];
				if (is_dirty(
...
function update_reaction(reaction) {
	...
	try {
		...
		if (new_deps !== null) {
			...
			if (!skip_reaction || (flags & DERIVED) !== 0 && reaction.reactions !== null) for (i = skipped_deps; i < deps.length; i++) (deps[i].reactions ??= []).push(reaction);
...
function remove_reaction(signal, dependency) {
	...
	if (reactions === null && (dependency.f & DERIVED) !== 0 && (new_deps === null || !new_deps.includes(dependency))) {
		...
		if ((dependency.f & (UNOWNED | DISCONNECTED)) === 0) dependency.f ^= DISCONNECTED;
...
function update_effect(effect) {
	...
	try {
		if ((flags & BLOCK_EFFECT) !== 0) destroy_block_effect_children(effect);
...
function get(signal) {
	...
	if (active_reaction !== null && !untracking) {
		...
		var derived = signal;
		var parent = derived.parent;
		if (parent !== null && (parent.f & UNOWNED) === 0) derived.f ^= UNOWNED;
	}
...
//#endregion
export { BOUNDARY_EFFECT, BROWSER, Batch, COMMENT_NODE, EFFECT_PRESERVED, EFFECT_TRANSPARENT, ELEMENT_IS_INPUT, ELEMENT_IS_NAMESPACED, ELEMENT_PRESERVE_ATTRIBUTE_CASE, HYDRATION_END, HYDRATION_ERROR, HYDRATION_START, HYDRATION_START_ELSE, LEGACY_PROPS, STALE_REACTION, active_effect, active_reaction, array_from, block, branch, clear_text_content, component_context, component_root, create_text, define_property, destroy_effect, effect_tracking, flushSync, get as get$1, get_first_child, get_next_sibling, handle_error, hydration_failed, increment, init_operations, internal_set, invoke_error_boundary, move_effect, mutable_source, noop, pause_effect, pop as pop$1, push as push$1, queue_micro_task, render_effect, run_all, safe_not_equal, set, set_active_effect, set_active_reaction, set_component_context, source, subscribe_to_store, svelte_boundary_reset_onerror, untrack };
//# sourceMappingURL=utils2-B3oc7zZR.js.map