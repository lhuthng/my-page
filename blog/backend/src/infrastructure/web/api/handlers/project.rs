...
use crate::{
    ...
    infrastructure::web::{
        ...
        api::handlers::v86::{V86RuntimeDescriptor, runtime_descriptor},
...
// Runs on every project create and update.
...
async fn extract_demo_zip(
    ...
    tokio::task::spawn_blocking(move || extract_demo_zip_blocking(config, project_id, zip_bytes))
        .await
        .map_err(|e| ProjectError::InternalError(e.to_string()))?
}
...
pub async fn new_project(
    ...
    if let Some(zip) = demo_zip {
        if let Err(err) = extract_demo_zip(&state.project_demo_config, project_id, zip).await {
            return Err(err);
        }
    }
...
pub async fn update_project(
    ...
    if let Some(zip) = parsed.demo_zip {
        extract_demo_zip(&state.project_demo_config, project_id, zip).await?;
    }
