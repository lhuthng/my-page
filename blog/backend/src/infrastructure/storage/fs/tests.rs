...
async fn put_get_roundtrip_and_missing_object() {
    ...
    store
        ...
        .unwrap();
    assert_eq!(
        store.get_object("v86/saves/1/2/save.zst").await.unwrap(),
        b"hello save"
    );
    assert_eq!(
    ...
    assert_eq!(
        store.object_size("v86/saves/1/2/missing").await.unwrap(),
        None
    );
...
async fn multipart_roundtrip_with_out_of_order_parts() {
    ...
    assert!(
        !root
            .join("v86/tmp/snapshots/upload-1.zst.multipart")
            .exists()
    );
    std::fs::remove_dir_all(root).ok();
}
...
async fn multipart_abort_removes_session() {
    ...
    assert!(
        !root
            .join("v86/tmp/snapshots/upload-2.zst.multipart")
            .exists()
    );
...
async fn delete_object_and_prefix_are_idempotent() {
    ...
    store
        ...
        .unwrap();
    store
        .delete_object("v86/games/sha1/full.iso")
        .await
        .unwrap();
    store
        .delete_object("v86/games/sha1/full.iso")
    ...
    assert_eq!(
        store
            .object_size("v86/games/sha1/0-4.img.zst")
            .await
            .unwrap(),
        None
    );
...
async fn get_object_range_reads_inclusive_bounds() {
    ...
    store
        .put_object_bytes(
            "v86/assets/systems/sha/base.img.zst",
            b"0123456789".to_vec(),
        )
        .await
        .unwrap();
...
async fn put_object_from_file_copies_content() {
    ...
    assert_eq!(
        store
            .get_object("v86/games/sha2/base.img.zst")
            .await
            .unwrap(),
        b"built artifact"
    );
