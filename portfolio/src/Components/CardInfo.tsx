...
function calculatePoints(
  ...
  return [
    points
      .map(
        (point, index) =>
          `${index === 0 ? "M" : "L"}${Number(point[0]).toFixed(3)} ${Number(point[1]).toFixed(3)}`,
      )
      .join(),
    {
      x: minX,
...
export default function CardInfo({
  ...
  return (
    <div className="absolute not-sm:-translate-y-13">
      <div
        style={{
          position: "absolute",
          left: `${pivot[0]}px`,
      ...
      >
        ...
        <div
          className={className + " not-lg:[&>div]:text-center"}
          ref={detailRef}
          style={{
            ...style,
            position: "absolute",
